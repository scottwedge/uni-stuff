...
First of all we need to check all the data for certain properties: Stationarity, Autocorrelation, Error-distribution, Data-distribution.\\
...
Before checking the data for the distribution we will build CDF (cumulative distribution function) and KDE (kernel density distribution) for each company from the data table [8][9].  
\\
\textcolor{blue}{an example for several companies CDF, KDE}
\\
As you can see, the empirical distribution differs a lot from the normal distribution. In order to prove it, we will run Kolmogorov's goodness of fit test for one sample [3], and compare the empirical distribution with normal distribution. Further, we will run KS-test (Kolmogorov's test) for three most "popular" distributions for financial data: lognormal, Levy stable and Pareto. The data from each company will be checked 4 times with confidence level of 15\%. If several tests accept the first hypothesis, tests will be run again with higher confidence level until the closer match is found.\\
According to the tests results our data is lon-normally distributed with 95\% confidence. This means that we can actually build a linear multiple regression and use the same procedure for Maximum Likelihood estimation as for normal distribution.  
\\
The time series of all companies are stationary and will be checked for Autocorrelation. Autocorrelation according to Wikipedia:"Autocorrelation, also known as serial correlation, is the cross-correlation of a signal with itself. Informally, it is the similarity between observations as a function of the time lag between them. It is a mathematical tool for finding repeating patterns, such as the presence of a periodic signal obscured by noise, or identifying the missing fundamental frequency in a signal implied by its harmonic frequencies"[6]. The data will be checked within our program with Breush-Godfrey test [7]. The same as with stationary-check: if the data is autocorrelated, than it is thrown out of further consideration. 
