...
After checking the data for stationary-processes we will check the hypothesis about distribution of the data. In order to get accurate linear regression the data should have the Normal distribution - this is a strict assumption for the model-class (see models overview). Several test will be run: Kolmogorov-Smirnov test, Normality test and Anderson-Darling test. If all three test will yield the same negative result (the hypothesis that the data has normal distribution is rejected), we will estimate second and third moment to understand how far from normal distribution is the empirical one. Afterwards another several tests for other distributions will be made. Until we find close distribution (not less than 90\% confidence level).
\\
