...
\begin{verbatim}
['ABB Robotics', 'Adept Technology', 'Bosch', 'Caterpillar', 'Denso Robotics', 'Fanuc Robotics', 'Foxconn', 'Google', 'Kuka Robotics', 'Marel hf', 'Rethink Robotics', 'RoboCV', 'Scape Technologies', 'SCHUNK', 'Seiko Epson', 'Universal Electronics', 'Stäubli']
\end{verbatim}
No data for: Fanuc Robotics (since 2010), Foxconn (no international trade), Kuka Robotics (trade volume 0), Marel hf (not traded), Rethink Robotics (london trade), RoboCV (not tarded), Scape Technologies (not traded), SCHUNK (not traded), Seiko Epson (since 2008), Stäubli (not traded).\\
...
Of course, these are not all companies that should be considered to establish a good model. But our first goal is to establish an more or less working model. We want our original model to be really bad and not flexible, that is why we have limited the number of variables to iterate through.
