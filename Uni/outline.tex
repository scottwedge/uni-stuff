...
The data for each company will be checked for stationary character of the process, autocorrelation, distribution of the data and error's distribution and independancy.\\
...
\begin{enumerate}
	\item The correlation vector is ranged in uprising order. And the company with the highest correlation coefficient is taken. For this company the LLR, p-value and R² are computed.
...
\end{enumerate} 
As the final result, the first algorithm will return the regression model in the following form: \[Dependant = \alpha + \sum_{i=0}^{N} \beta_{i}Comp_{i}\] 
