		...
		\begin{enumerate}
			...
			\item[2.1] Data preparation (correlation matrices and further filtering)	
				\begin{enumerate}
					...
					\item[2.1.2] Splitting the data
					\item[2.1.3] Cut-off: first- and second-order
				\end{enumerate}
...
\subsubsection{First- and second-order data cut-off}
...
The goal of this bachelor work is to check the ability of sentimental data analysis to fine-tune forecasting models. This is why we will take a pretty bad forecasting model, which is multiple linear regression. On the other hand the estimating mechanism for this type of regression is very easy and transparent to program.\\
Currently we will land into back-pack problem that normally have n! complexity. This is the second worst complexity-class that we possibly could have chosen. This is why the number of the companies in the linear regression will be reduced to a very small number: let's say 5 companies. With restriction to 5 companies we have to check \[\binom{n}{k} = \frac{n!}{k!(n-k)!}\]
