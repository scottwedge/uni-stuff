...
the standart deviation is: 1.35949376438
\end{verbatim}
\includegraphics[scale=0.75]{PythonPredictions.png}
As you can see, the results of the Python Program yield similar trend to the real time series, but the absolute difference is bigger, than by R program. Since we didn't get the source code for the R nlme library, we can't say whether the implementation of the computations differ. We have seen that the algorithms yield the same reault until the last step. We have also noticed that the coefficients differ from program to program a bit. It might be the case that on the 6th step the differences became significant and the result became different. On the next graph you can see both predictions together compared with the real prices.
\includegraphics[scale=0.75]{BothPredictions.png}
textcolor{red}{calculate accumulated error for both programs}
give 1 point to the program with the closer results.
Compare final score.
