...
\begin{enumerate}
    \item Python Standart Library [30].
...
\subsubsection{Documentation and The Ecosystem}
Originally, Python was used a lot for scientific purposes. This is why many libraries have specific formatting and high complexity. As we have already mentioned earlier, in this work we have used Python 3 standard library,  SciPy and Statmodels.\\
...
\begin{enumerate}
    ...
    \item 0 Point.
\end{enumerate}
Additionally to the score for the documentation only we will give Python 1 Point for the size and the quality of the community. We give the whole point because there are many serious universities developing Python. One of the interesting examples is the library to place graphs directly into web [33]. Seaborn is developed and maintained by Standford. Another important example is TensorFlow by Google [34]. This tool offers Java and Python APIs to build and train the neural network.\\
...
The second advantage of the language is testing. Python has a built-in doc-tests. Another possibility to test a program are Unit Tests. Python provides different ways to test and to debug the program in order to avoid possible computational and logic errors. We will not discuss this theme in details since the tutorials and the documentation are simple and offer sufficient amount of examples.\\
...
The community behind R is more scientific than behind Python. Hoverer there are more scientific domain packages that R community has developed. Additionally, to implement certain algorithm there can be more the one package available. The language gets 1 Point for the community size and quality criterion and 1 Point for the diversity of the packages. The final score for R for this section is 2.591.
...
The first disadvantage of the R language is difficulty. The syntax is not as intuitive as the Python syntax. The data structures and their use are non trivial. In order to begin using R significant amount of time invested upfront is needed.\\
...
After point to point comparison and collecting the personal opinion, we have decided, that R programming language is bit more suitable for a pure analytical task.
\begin{enumerate}
...
Our conclusion suits the modern trend which says that R is very popular among the beginners and advanced analysts. However R programmers turn slowly to Python due to its multipurpose nature and according freedom.
